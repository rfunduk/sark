defmodule Sark.LLM.Stub do
  @moduledoc """
  Deterministic LLM stub for tests.

  Drive an `llm:` step (or any agent loop) without hitting Anthropic by
  pre-loading a script of turns. Each call to `chat/1` pops the next
  turn from the script and returns it as a canonical `Response`.
  ...
  Usage:
      ...
      {:ok, _} = Sark.LLM.Stub.start_link(script)

      Sark.LLM.Stub.chat(%{model: "x", messages: []})

  Also records every `chat/1` call so tests can assert on what the
  caller sent (messages, tools, system prompt). Inspect via
  `recorded_calls/0`.
  """
