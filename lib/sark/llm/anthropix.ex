defmodule Sark.LLM.Anthropix do
  ...
  are stable across turns within a single pipeline run, so subsequent
  turns hit the cache (5m TTL — pipelines with long cadences won't
  carry cache hits across runs).

  Streaming: every call uses SSE streaming under the hood. Pipelines
  ...
  passed through the same decoder as the non-streaming path. Streaming
  removes the ~10min non-streaming request ceiling (matters once a
  pipeline starts using extended thinking or emits long final outputs).
  """
