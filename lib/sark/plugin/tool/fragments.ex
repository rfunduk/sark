defmodule Sark.Plugin.Tool.Fragments do
  @moduledoc """
  Resolve `@name` fragment references against a `shared:` map.

  `shared:` blocks from `plugin.yml` and any included files are merged
  into one map (dupe key across files raises). Each entry (tool or
  pipeline) is walked top-to-bottom; any string starting with `@` is
  treated as a reference and replaced with the corresponding `shared`
  value.
