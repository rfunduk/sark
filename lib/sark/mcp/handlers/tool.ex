defmodule Sark.MCP.Handlers.Tool do
  @moduledoc """
  Runtime dispatcher for canned MCP tools.

  A tool may have one or more SQL statements (`sql:` accepts a string
  ...
  defp do_call(plugin, tool_name, raw_params, session, opts) do
    ...
    with {:ok, %Tool{} = q} <- Registry.get(plugin, tool_name),
      ...
      :error ->
        reply_error("tool not found: #{plugin}.#{tool_name}", session)
  ...
  defp check_rejects(%Tool{reject: []}, _coerced, _runner), do: :ok

  defp check_rejects(%Tool{reject: rejects, params: params}, coerced, runner) do
  ...
  defp run_writes(conn, %Tool{statements: statements, returns: returns}, binds) do
  ...
  defp reply_text(str, session), do: {:reply, Reply.text(str), session}
